/**
 * MenuAnalysisTab - Menu Engineering analysis component
 *
 * Extracted from MenuEngineeringPage for use as a tab within EscandallosPage
 * Session 002: Fusion of Escandallos + Ingenieria Menu
 */
...
export const MenuAnalysisTab: React.FC = () => {
  ...
  const menuAnalysis = useMemo(() => {
    // Escandallo type has no 'fecha' field; filter by createdAt if available
    ...
    return relevantEscandallos.map(escandallo => {
      ...
      if (isHighMargin && isHighPopularity) {
        ...
        categoria = 'puzzle';
        recomendacion = 'Promocionar mas. Rentable pero poco conocido.';
      } else {
        categoria = 'dog';
  ...
  const exportAnalysis = () => {
    const csv = [
      ['Nombre', 'Categoria', 'Margen %', 'Margen EUR', 'Popularidad', 'Ventas Est.', 'Ingresos Tot.', 'Recomendacion'],
    ...
    showToast({
      ...
      message: 'El analisis ha sido exportado correctamente',
    });
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-lg)', marginTop: 'var(--spacing-md)' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 'var(--spacing-md)' }}>
        <div>
          <p style={{ margin: 0, color: 'var(--text-secondary)', fontSize: 'var(--font-size-base)' }}>
            Analisis de rentabilidad y popularidad de platos (Matriz Stars/Dogs)
          </p>
        </div>
      ...
      <Card>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 'var(--spacing-md)' }}>
          ...
          <Select label="Categoria" value={categoryFilter} onChange={(v) => setCategoryFilter(v as CategoryFilter)} options={[
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 'var(--spacing-md)' }}>
        {(['star', 'puzzle', 'plow_horse', 'dog'] as const).map(cat => (
          <Card key={cat} style={{ border: `2px solid ${getCategoryColor(cat)}` }}>
      ...
      <Card>
        <Table
          ...
          columns={[
            {
              key: 'categoria', header: 'Categoria', render: (_, item) => (
            ...
            { key: 'margenBruto', header: 'Margen EUR', render: (_, item) => formatCurrency(item.margenBruto), sortable: true },
            {
